      ...
      <div className='basis-1/3 min-w-0'>
        ...
        <ul className='mt-1'>
          {listItems.map((item, key) => {
            const listItem = (key: number, invisibleFillingItem: boolean) => {
              const lastPageDisplayed = listFirstItemIndex + itemsPerPage > listItems.length;
              return (
                <li
                  className={
            ...
            const isLastItem = key === listItems.length - 1;
            //const isLastPage = listFirstItemIndex + itemsPerPage > listItems.length;
            const isLastPageIncomplete = listItems.length % itemsPerPage !== 0;
            if (isLastItem && isLastPageIncomplete) {
              const invisibleItemCount = itemsPerPage - (listItems.length % itemsPerPage) + 1;
        ...
        <div className='flex flex-row justify-center mb-8'>
          <ul className='flex flex-row flex-wrap items-center'>
            {Array(Math.ceil(listItems.length / itemsPerPage))
              .fill(null)
              .map((item, key) => (
                <li
                  key={key}
                  className={cn(
                    "p-3 max-sm:p-5 border-t-[3px] border-transparent",
                    {
                      "border-blue-400": listFirstItemIndex === key * itemsPerPage,
                    },
                    { "hover:border-slate-300": listFirstItemIndex !== key * itemsPerPage }
                  )}
